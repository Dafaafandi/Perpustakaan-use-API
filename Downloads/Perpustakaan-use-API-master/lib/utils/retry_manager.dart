...
class RetryManager {
  ...
  static Future<T> executeWithRetry<T>(
    ...
    while (attemptCount <= maxRetries) {
      try {
        if (kDebugMode && context != null && attemptCount > 0) {
          print('$context: Retry attempt $attemptCount/$maxRetries');
        }

        final result = await operation();

        if (kDebugMode && context != null && attemptCount > 0) {
          print('$context: Succeeded after $attemptCount retries');
        }
        ...
        if (kDebugMode && context != null) {
          print('$context: Attempt ${attemptCount + 1} failed: $e');
        }
        ...
        if (retryIf != null && !retryIf(lastException)) {
          if (kDebugMode && context != null) {
            print('$context: Retry condition not met, stopping retries');
          }
          break;
        }
        ...
        if (!_isRetryableError(lastException)) {
          if (kDebugMode && context != null) {
            print('$context: Non-retryable error, stopping retries');
          }
          break;
        }
        ...
        if (kDebugMode && context != null) {
          print('$context: Waiting ${delay.inMilliseconds}ms before retry');
        }
    ...
    if (kDebugMode && context != null) {
      print('$context: All retries failed, throwing last exception');
    }
  ...
  static Future<bool> executeBoolWithRetry(
    ...
    try {
      ...
      if (kDebugMode && context != null) {
        print('$context: Operation failed completely: $e');
      }
      return false;
    }
  }
